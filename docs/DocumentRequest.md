# SignWell::Models::DocumentRequest
...
| **recipients** | [**Array&lt;RecipientsInner&gt;**](RecipientsInner.md) | Document recipients are people that must complete and/or sign a document. |  |
...
| **self_sign** | **Boolean** | When set to true the document will enter self-sign mode, which allows the document creator to sign and complete a document without any field setup. Must use the embedded_edit_url in the response and (if embedding) the embedded requesting workflow. | [optional][default to false] |
...
| **fields** | **Array&lt;Array&lt;FieldsInnerInner&gt;&gt;** | Document fields placed on a document for collecting data or signatures from recipients. At least one field must be present in the Create Document request if &#x60;draft&#x60; is &#x60;false&#x60; (unless adding a signature page by using &#x60;with_signature_page&#x60;). Field data should be sent as a 2-dimensional JSON array. One array of fields is needed for each file in the files array. An array of fields can be empty if you have a file that does not contain any fields. | [optional] |
...
instance = SignWell::Models::DocumentRequest.new(
  test_mode: null,
  files: null,
  name: null,
  subject: null,
