    ...
    class DocumentResponseFieldsInnerInner
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          fail ArgumentError, "The input argument (attributes) must be a hash in `SignWell::DocumentResponseFieldsInnerInner` initialize method"
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!acceptable_attribute_map.key?(k.to_sym))
            fail ArgumentError, "`#{k}` is not a valid attribute in `SignWell::DocumentResponseFieldsInnerInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end
          h[k.to_sym] = v
        }
      ...
      def normalize_for_serialization!
        return unless type == 'checkbox' && !value.nil?

        self.value = normalize_checkbox_value(value)
      end

      def normalize_checkbox_value(raw_value)
        actual_value = raw_value.respond_to?(:actual_instance) ? raw_value.actual_instance : raw_value
        return 't' if actual_value == true
        return 'f' if actual_value == false

        if actual_value.is_a?(String)
          normalized = actual_value.strip.downcase
          return 't' if %w[true t].include?(normalized)
          return 'f' if %w[false f].include?(normalized)
        end

        fail ArgumentError, 'Checkbox field values must be boolean or one of "true", "false", "t", or "f".'
      end
      ...
      def hash
        [x, y, page, recipient, api_id, name, date_format, fixed_width, formula, label, lock_sign_date, required, type, validation, value, height, width, recipient_id, signing_elements_group_id, placeholder_name, options, default_option, allow_other].hash
      end
