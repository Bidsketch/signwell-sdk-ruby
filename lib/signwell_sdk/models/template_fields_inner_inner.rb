    ...
    class TemplateFieldsInnerInner
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          fail ArgumentError, "The input argument (attributes) must be a hash in `SignWell::TemplateFieldsInnerInner` initialize method"
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!acceptable_attribute_map.key?(k.to_sym))
            fail ArgumentError, "`#{k}` is not a valid attribute in `SignWell::TemplateFieldsInnerInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end
          h[k.to_sym] = v
        }
        ...
        if attributes.key?(:'placeholder_id')
      ...
      def placeholder_id=(placeholder_id)
        if placeholder_id.nil?
          fail ArgumentError, 'placeholder_id cannot be nil'
        end
      ...
      def hash
        [x, y, page, placeholder_id, type, required, label, value, api_id, name, validation, fixed_width, lock_sign_date, date_format, height, width, options, default_option, allow_other].hash
      end
