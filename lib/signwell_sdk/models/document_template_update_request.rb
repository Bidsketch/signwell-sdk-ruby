    ...
    class DocumentTemplateUpdateRequest
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          fail ArgumentError, "The input argument (attributes) must be a hash in `SignWell::DocumentTemplateUpdateRequest` initialize method"
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!acceptable_attribute_map.key?(k.to_sym))
            fail ArgumentError, "`#{k}` is not a valid attribute in `SignWell::DocumentTemplateUpdateRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
          end
          h[k.to_sym] = v
        }
      ...
      def hash
        [name, subject, message, draft, expires_in, reminders, apply_signing_order, api_application_id, redirect_url, allow_decline, allow_reassign, decline_redirect_url, metadata, labels, checkbox_groups].hash
      end
