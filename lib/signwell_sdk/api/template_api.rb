...
module SignWell
  module Resources
    class TemplateApi
      ...
      # @option opts [String] :query Raw API filter query. Use AND between filters, for example: \&quot;name:Classic AND status:Available\&quot;.
      ...
      def list_templates_with_http_info(opts = {})
        ...
        if @api_client.config.client_side_validation && !opts[:'page'].nil? && opts[:'page'] < 1
          fail ArgumentError, 'invalid value for "opts[:"page"]" when calling TemplateApi.list_templates, must be greater than or equal to 1.'
        end

        if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 50
          fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling TemplateApi.list_templates, must be smaller than or equal to 50.'
        end
        ...
        if @api_client.config.client_side_validation && !opts[:'query'].nil? && opts[:'query'].to_s.empty?
          fail ArgumentError, 'invalid value for "opts[:"query"]" when calling TemplateApi.list_templates, the character length must be great than or equal to 1.'
        end
      ...
      # Iterate over paginated list_templates responses.
      def iterate_template_pages(opts = {})
        return enum_for(:iterate_template_pages, opts) unless block_given?

        next_page = opts[:page] || 1
        loop do
          response = list_templates(opts.merge(page: next_page))
          yield response
      ...
      # Iterate over all list_templates items.
      def iterate_templates(opts = {}, &block)
        return enum_for(:iterate_templates, opts) unless block_given?

        iterate_template_pages(opts) do |page_response|
          Array(page_response.respond_to?(:templates) ? page_response.templates : []).each(&block)
        end
      end
