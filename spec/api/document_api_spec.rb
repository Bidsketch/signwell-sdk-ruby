...
RSpec.describe SignWell::Resources::DocumentApi, :prism do
  ...
  describe '#get_completed_pdf' do
    it 'returns CompletedPdfResponse' do
      result = @api.get_completed_pdf(@test_id, { url_only: true })
      expect(result).not_to be_nil
    end
  end
  ...
  describe '#update_recipients' do
    it 'returns DocumentResponse' do
      body = SignWell::UpdateRecipientsRequest.new({ recipients: [SignWell::UpdateRecipientsMapInner.new({ id: 'id_example', name: 'name_example', email: 'jane@example.com' })] })
