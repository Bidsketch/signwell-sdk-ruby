# frozen_string_literal: true

# SignWell Developer API
#
# API for creating, managing, and tracking electronic signature workflows.
#
# The version of the OpenAPI document: v1
# Contact: support@signwell.com
# Generated by: https://openapi-generator.tech
# Generator version: 7.12.0
#

require 'spec_helper'
require 'json'
require 'date'

# Unit tests for SignWell::Models::ApiApplicationResponse
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe SignWell::Models::ApiApplicationResponse do
  let(:instance) { SignWell::Models::ApiApplicationResponse.allocate }

  describe 'test an instance of ApiApplicationResponse' do
    it 'should create an instance of ApiApplicationResponse' do
      expect(instance).to be_a(SignWell::Models::ApiApplicationResponse)
    end
  end

  describe 'test attribute "id"' do
    it 'should work' do
      expect(instance).to respond_to(:id)
    end
  end

  describe 'test attribute "name"' do
    it 'should work' do
      expect(instance).to respond_to(:name)
    end
  end

  describe 'test attribute "callback_urls"' do
    it 'should work' do
      expect(instance).to respond_to(:callback_urls)
    end
  end

  describe 'test attribute "created_at"' do
    it 'should work' do
      expect(instance).to respond_to(:created_at)
    end
  end

  describe 'test attribute "updated_at"' do
    it 'should work' do
      expect(instance).to respond_to(:updated_at)
    end
  end

  describe 'test attribute "owner"' do
    it 'should work' do
      expect(instance).to respond_to(:owner)
    end
  end

  describe 'test attribute "preferences"' do
    it 'should work' do
      expect(instance).to respond_to(:preferences)
    end
  end
end
